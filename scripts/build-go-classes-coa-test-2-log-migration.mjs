...
const QUESTION_PATH = path.join(SCRIPT_DIR, 'pyq-custom', 'go-classes-coa-topic-test-2.json');
const ATTEMPT_PATH = path.join(
  SCRIPT_DIR,
  'pyq-custom',
  'go-classes-coa-topic-test-2-attempts.json'
);
const OUTPUT_PATH = path.join(
  ROOT,
  'supabase',
  'migrations',
  '20260813000003_go_classes_coa_topic_test_2.sql'
);
const IMPORT_KEY = 'goclasses-coa-topic-test-2';
...
function plainText(html) {
  ...
  for (const list of document.querySelectorAll('ol')) {
    ...
    const upperAlpha = style.includes('upper-alpha');
    const lowerRoman = style.includes('lower-roman');
    items.forEach((item, index) => {
      const prefix = upperAlpha
  ...
  for (const row of document.querySelectorAll('tr')) row.append('\n');
  for (const cell of document.querySelectorAll('th, td')) cell.append(' | ');
  for (const element of document.querySelectorAll('p, pre, div, ul, ol, table')) {
    element.append('\n');
  }
...
const rows = attemptPayload.questions.map((attempt) => {
  ...
  const attemptId = uuidFromString(`${IMPORT_KEY}:attempt:${question.number}`);
  const journalQuestionId = uuidFromString(`pyq-journal-question:${attemptId}`);
  const reattemptId = uuidFromString(`${IMPORT_KEY}:reattempt:${question.number}`);
  const row = {
    attemptId,
    journalQuestionId,
    reattemptId,
...
for (const row of rows) {
  ...
  if (answered && row.markCorrect !== sameAnswer(row.selectedAnswer, row.question.answer)) {
    throw new Error(`Question ${row.number} has an incorrect correctness flag`);
  }
}

const answerSql = (jsonPath) => `case
        when ${jsonPath} = 'null'::jsonb then 'Not attempted'
        when jsonb_typeof(${jsonPath}) = 'array'
          then array_to_string(array(select jsonb_array_elements_text(${jsonPath})), ', ')
        else ${jsonPath} #>> '{}'
      end`;

const selectedAnswerTextSql = answerSql("item->'selectedAnswer'");
const correctAnswerTextSql = answerSql("item#>'{question,answer}'");

const sql = `-- Import the learner-provided GO Classes COA Topic Test 2 into the
-- owner's immutable PYQ receipts and journal. Each receipt preserves both the
-- learner response and official key; fixed IDs make the import idempotent.
do $migration$
declare
  target_user_id uuid;
  item jsonb;
begin
  ...
  loop
    insert into public.pyq_attempts (
      ...
      1,
      case when item->'selectedAnswer' = 'null'::jsonb then null else item->'selectedAnswer' end,
    ...
    insert into public.questions (
      ...
      item#>>'{question,topic}',
      (item#>>'{question,year}')::int,
      (item#>>'{question,paperLabel}') || ' · Q' || (item->>'number') || ' · ' || (item#>>'{question,type}'),
      item->>'questionText',
      'Your answer: ' || ${selectedAnswerTextSql} || E'\\nActual answer: ' || ${correctAnswerTextSql},
      null,
      (item->>'timeSpentSec')::int,
