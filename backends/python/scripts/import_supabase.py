...
def external_id(collection: str, row: dict[str, Any]) -> str:
    ...
    natural_keys = {
        ...
        "topic_progress": ("subject", "topic"),
        "readiness_snapshots": ("snapshot_date",),
        "push_subscriptions": ("device_id",),
        "study_notification_preferences": (),
        "telegram_subscriptions": (),
    }
    keys = natural_keys.get(collection)
    if keys is None or any(row.get(key) is None for key in keys):
        raise ValueError(f"No verified stable identity for {collection}; refusing import")
    stable = json.dumps([row[key] for key in keys], default=str)
...
async def import_rows(source_url: str, user_map: dict[str, str], apply: bool) -> None:
    if apply:
        raise RuntimeError(
            "Production import is disabled: shared data, identity reconciliation, "
            "and attachment verification are not yet complete. Source data is unchanged."
        )
    if len(set(user_map.values())) != len(user_map):
        raise ValueError("Identity map must be one-to-one; merging accounts is unsafe")
    source = await asyncpg.connect(source_url)
    counts: dict[str, int] = {}
    try:
        await source.execute("BEGIN ISOLATION LEVEL REPEATABLE READ READ ONLY")
            ...
            for collection in OWNED_COLLECTIONS:
                ...
                for source_row in source_rows:
                    ...
                    if old_owner is None:
                        raise RuntimeError(f"Unowned {collection} row requires explicit migration")
    ...
    if not apply:
        print("Partial dry run only. --apply is disabled until full migration parity is verified.")
