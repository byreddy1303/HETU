...
from app.db.models import BackgroundJob, FileObject
...
async def execute_job(db: AsyncSession, job: BackgroundJob) -> None:
    ...
    try:
        if job.kind == "realtime-event":
            ...
            # Also neutralizes destructive jobs queued by an older deployment.
            raise ValueError("Automatic user purge disabled; reviewed erasure required")
        elif job.kind == "noop":
            pass
        else:
